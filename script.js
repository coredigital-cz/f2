...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.gsap) {
    ...
    gsap.utils.toArray('.rv').forEach(el => {
      if (el.closest('.hero') || el.classList.contains('card') || el.classList.contains('split-media') || el.classList.contains('g-item') || el.classList.contains('company-card')) return;
    ...
    gsap.utils.toArray('.company-card').forEach(el => {
      gsap.fromTo(el, { opacity: 0, y: 40, scale: .95 }, {
        opacity: 1, y: 0, scale: 1, duration: .8, ease: 'back.out(1.2)',
        scrollTrigger: { trigger: el, start: 'top 88%' }
      });
    });
    ScrollTrigger.batch('.company-fact', {
      start: 'top 90%',
      onEnter: b => gsap.fromTo(b, { opacity: 0, y: 14 }, { opacity: 1, y: 0, duration: .5, stagger: .07, delay: .3, ease: 'back.out(1.6)', overwrite: true })
    });
